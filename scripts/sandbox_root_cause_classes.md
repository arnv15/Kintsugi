# Sandbox Root Cause Classes

This authoring note is intentionally excluded from the `baseline` tag so it
cannot reveal a diagnosis or fix strategy to a Run.
...
| `asyncio` exception semantics | Async completion or failure is hidden from the caller, so an unsuccessful operation is treated as a successful result. | Await the operation at the public completion boundary without suppressing its exception or unfinished state. |
